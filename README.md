...
| 45 | 2026-08-04 | [py-pdf/pypdf](https://github.com/py-pdf/pypdf) | **Bug fix** — follow-up to #3929: low-bit expansion only ran for FlateDecode, so unfiltered/inline images passed a raw `"4bits"` mode to Pillow and raised `unrecognized image mode`; extracted `_expand_low_bit_samples()` and applied it to all three raw-bytes paths | [#3938](https://github.com/py-pdf/pypdf/pull/3938) | ⏳ Open |
